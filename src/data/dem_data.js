...
export default class DEMData {
    ...
    constructor(uid: string, data: RGBAImage, encoding: "mapbox" | "terrarium" | "gsi") {
        this.uid = uid;
        ...
        const unpack = encoding === "terrarium" ? this._unpackTerrarium :
            encoding === "mapbox" ? this._unpackMapbox : this._unpackGsi;
        for (let y = 0; y < dim; y++) {
            for (let x = 0; x < dim; x++) {
                const i = y * dim + x;
                const j = i * 4;
